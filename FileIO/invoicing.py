...
def parse_invoice_number(invoice_number: str) -> tuple[int, int]:
    ...
    # return tuple(int(num) for num in split)  # my incomplete solution

    year, number = invoice_number.split('-')
    return int(year), int(number)       # Tim's solution
...
def next_invoice_number(invoice_number: str) -> str:
    ...
    #     if current_year == invoice_number:
    #         value += 1
    #     else:
    #         value = 1
...
def record_invoice(invoice_file: TextIO,
    ...
    for line in invoice_file:
        ...
        last_row = line
    if last_row:
        # # using underscore(_) to variables that won't be used
        ...
        year = get_year()
        new_invoice_number = f'{year}-{1:04d}'  # :04d to create values of the type 000X
